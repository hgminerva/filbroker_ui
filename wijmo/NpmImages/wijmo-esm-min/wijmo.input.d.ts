...
import { Control, CancelEventArgs, Event, EventArgs, Binding, ICollectionView, CollectionViewGroup, _ClickRepeater, CollectionView, _MaskProvider } from 'wijmo/wijmo';
...
/**
 * The {@link CollectionViewNavigator} control provides a UI
 * for navigating through the data items or pages in a
 * {@link CollectionView} object.
 *
 * Use the navigator's {@link cv} property to bind it to a
 * {@link CollectionView}, and the {@link byPage} property
 * to define whether the navigator should show data items
 * or pages.
 *
 * The navigator shows VCR-like buttons that allow users to
 * select the first/previous/next/last data item (or page)
 * in the {@link CollectionView}, and some text showing
 * the current index and total count.
 *
 * You may use the {@link headerFormat} property to customize
 * the text displayed by the navigator.
 */
...
export declare class Popup extends Control {
    ...
    onLostFocus(e?: EventArgs): void;
    refresh(fullUpdate?: boolean): void;
    _handleDragResize(on: boolean): void;
